...
export function Mails(props) {
    ...
    const indexOfFristMail = indexOfLastMail - mailPerPage;
    const history = useHistory();
    const token = document.cookie.match(/(^|;) ?token=([^;]*)(;|$)/)[2];

    useEffect(() => {
        // console.log("isEditingMail after update", isEditingMail);
    }, [isEditingMail]);
    ...
    const handleAddMailSubmit = async (event) => {
        ...
        const formData = new FormData(document.getElementById('addMailForm')); // Récupération des données dans le formulaire
    ...
    useEffect(() => {
        // console.log("mailIndex after update", mailIndex);
    }, [mailIndex]);

    const handleUpdate = async () => {
        const response = await fetch(`http://localhost:8000/api/updateMail/${companyData.mail[mailIndex]._id}`, {
            method: 'PATCH',
            headers: {
                "Content-Type": "application/json",
                'Authorization': `Bearer ${token}`
            ...
            body: JSON.stringify({
                mail: nameMail || companyData.mail[mailIndex].mail,
                description_mail: descriptionMail || companyData.mail[mailIndex].description_mail
            })
        });
        if (response.ok) {
    ...
    const handleDelete = async () => {
        ...
        const response = await fetch(`http://localhost:8000/api/deleteMail/${companyData.mail[mailIndex]._id}`, {
            method: "DELETE",
            headers: {
                "Content-Type": "application/json",
                'Authorization': `Bearer ${token}`
            ...
            {isAddingMail ? (
                <div className="addMail">
                    <div className="header-addMail">
                        <h1>Ajouter un mail pour {companyData.name_company}</h1>
